...
//#define CONFIG_FT5406_UPGRADE
#if defined (CONFIG_FT5406_UPGRADE)
...
static u8 CTPM_FW[]=
{
//#include "FT5406_V14_1024x768_FW.h"//"ft_app.i"
//#include "ft5x06_all.bin"
#include "ft5406_api_fwr.i"
};
...
static void ft5406_queue_work(struct work_struct *work)
{
    ...
    static int i2c_reset_cnt=0;
    static int i2c_ignore_cnt[MAX_POINT]={0};
    ...
    for(i=0;i<points;i++){
        ...
        event.pressure = 200;
        temp_value = event.y * SCREEN_MAX_Y;
        temp_value = temp_value / 1024;
        event.y = temp_value;

        temp_value = event.x * SCREEN_MAX_X;
        temp_value = temp_value / 600;
        event.x = temp_value;
            
        if(event.x<=SCREEN_MAX_X && event.y<=SCREEN_MAX_Y){
            //printk(KERN_NOTICE "ft5406 multiple report event[%d]:x = %d,y = %d,id = %d,flag = %d,pressure = %d\n", 
    ...
    for(i=0;i<MAX_POINT;i++)
    {
          if((current_events[i].flag == 0) && (points_last_flag[i] != 0))
            ...
            {   
                ...
                input_report_abs(data->input_dev, ABS_MT_POSITION_X,  SCREEN_MAX_X-current_events[i].x);
                input_report_abs(data->input_dev, ABS_MT_POSITION_Y,  SCREEN_MAX_Y-current_events[i].y);    
...
static irqreturn_t ft5406_interrupt(int irq, void *dev_id)
{
    ...
    __cancel_delayed_work(&ft5x0x_ts->detect_work);
    disable_irq_nosync(ft5x0x_ts->irq);
    if (!work_pending(&ft5x0x_ts->pen_event_work)) 
        queue_work(ft5x0x_ts->ts_workqueue, &ft5x0x_ts->pen_event_work);
    schedule_delayed_work(&ft5x0x_ts->detect_work,msecs_to_jiffies(30000));    // 30s
    return IRQ_HANDLED;
}


static void detect_ts_work(struct work_struct * work)
{
    struct ft5x0x_ts_data *data = container_of(work, struct ft5x0x_ts_data, detect_work.work);
    struct ft5406_platform_data *pdata = data->client->dev.platform_data;
    if (pdata->platform_wakeup)                              
        pdata->platform_wakeup();
...
static int  ft5406_probe(struct i2c_client *client ,const struct i2c_device_id *id)
{
    ...
    //set_bit(ABS_MT_WIDTH_MAJOR, input_dev->absbit);i
    ...
    INIT_DELAYED_WORK(&ft5x0x_ts->detect_work,detect_ts_work);
    buf_w[0] = 6;
